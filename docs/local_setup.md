...
- `GET /api/admin/llm/scenario-packages` / `POST /api/admin/llm/scenario-packages` – admin CRUD for scenario graph packages with per-game versioning and activation. Admin can provide explicit graph `transitions` (`fromStepId`, `toStepId`, `condition`, `priority`) for branch routing. If `transitions` are omitted backend auto-links steps by ascending `order` and applies the target step `entryCondition` as the generated transition condition.
